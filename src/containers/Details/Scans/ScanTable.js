    ...
    if (rawRows) {
      ...
      _.map(rawRows, function(item) {
        ...
        let row = {
          ...
          progress: (
            <Tag className="some-class" type="green">
              {`${item.done_step}/${item.total_steps}`}
            </Tag>
          ),
...
export default inject('sessStore', 'axiosStore')(observer(ScanTable));
